...
public class AdjuntoController {
    ...
    @PostMapping("/seccion")
    public AdjuntoDTO saveToSeccion(
            @RequestParam("file") MultipartFile file,
            @RequestParam("descripcion") String descripcion,
            @RequestParam("seccionId") Long seccionId) {

        return adjuntoService.saveToSeccion(file, descripcion, seccionId);
    }

    @PostMapping("/encabezado")
    public AdjuntoDTO saveToEncabezado(
            @RequestParam("file") MultipartFile file,
            @RequestParam("descripcion") String descripcion,
            @RequestParam("encabezadoId") Long encabezadoId) {
        return adjuntoService.saveToEncabezado(file, descripcion, encabezadoId);
    }
}